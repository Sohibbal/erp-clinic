...
const DATE_RANGES = [
  { label: 'Oct 01, 2023 - Oct 31, 2023', revenue: 124500, patients: 1284, change: '+12.5%' },
  { label: 'Sep 01, 2023 - Sep 30, 2023', revenue: 110700, patients: 1156, change: '+8.2%' },
  { label: 'Aug 01, 2023 - Aug 31, 2023', revenue: 102300, patients: 1089, change: '+5.1%' },
];
    ...
    <div className="p-margin max-w-container-max mx-auto w-full">
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-stack-lg gap-4">
        <div>
          ...
          <p className="font-body-md text-body-md text-on-surface-variant">Track your clinical performance and clinic health.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-gutter mb-stack-lg">
        <div className="glass-card ambient-shadow p-6 rounded-2xl relative overflow-hidden group">
          ...
          <p className="font-label-md text-label-md text-outline uppercase mb-1">Total Revenue</p>
          <div className="flex items-end gap-2">
            <h3 className="font-headline-md text-headline-md text-on-surface">${range.revenue.toLocaleString()}</h3>
        ...
        <div className="glass-card ambient-shadow p-6 rounded-2xl">
          <p className="font-label-md text-label-md text-outline uppercase mb-1">Total Patients</p>
          <div className="flex items-end gap-2">
            <h3 className="font-headline-md text-headline-md text-on-surface">{range.patients.toLocaleString()}</h3>
            <span className="text-tertiary font-label-md mb-1">Active</span>
          </div>
        ...
        <div className="glass-card ambient-shadow p-6 rounded-2xl">
          <p className="font-label-md text-label-md text-outline uppercase mb-1">Top Service</p>
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-body-sm text-body-sm text-on-surface-variant">{OWNER_STATS.topService.sessions} sessions</p>
            </div>
          </div>
        </div>

        <div className="glass-card ambient-shadow p-6 rounded-2xl">
          <p className="font-label-md text-label-md text-outline uppercase mb-1">Inventory Health</p>
          <div className="flex items-center gap-3">
            <div className="flex flex-col">
              <h3 className="font-headline-sm text-headline-sm text-on-surface">{OWNER_STATS.inventoryHealth}</h3>
              <div className="flex items-center gap-1 mt-1">
                <div className="w-2 h-2 rounded-full bg-green-500"></div>
                <span className="font-body-sm text-body-sm text-on-surface-variant">All items in stock</span>
              </div>
            </div>
      ...
      <section className="glass-card ambient-shadow p-8 rounded-3xl mb-stack-lg">
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex gap-4">
            <div className="flex items-center gap-2"><span className="w-3 h-3 rounded-full bg-primary"></span><span className="font-label-md text-label-md">Current Period</span></div>
            <div className="flex items-center gap-2"><span className="w-3 h-3 rounded-full bg-outline-variant"></span><span className="font-label-md text-label-md">Previous Period</span></div>
          </div>
        </div>
          ...
          <svg className="w-full h-full" preserveAspectRatio="none" viewBox="0 0 1000 200">
            <defs>
              <linearGradient id="chartGradient" x1="0" x2="0" y1="0" y2="1">
                <stop offset="0%" stopColor="#7b5455" stopOpacity="0.2"></stop>
                <stop offset="100%" stopColor="#7b5455" stopOpacity="0"></stop>
              </linearGradient>
            </defs>
            ...
            <path d="M0,180 C100,160 200,40 300,60 C400,80 500,140 600,120 C700,100 800,20 900,40 L1000,60" fill="none" stroke="#7b5455" strokeLinecap="round" strokeWidth="3"></path>
            <circle cx="300" cy="60" fill="#7b5455" r="4"></circle>
            <circle cx="900" cy="40" fill="#7b5455" r="4"></circle>
          </svg>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-gutter mb-stack-lg">
        <div className="glass-card ambient-shadow p-8 rounded-3xl">
          <h3 className="font-headline-sm text-headline-sm text-on-surface mb-6">Top Services by Revenue</h3>
        ...
        <div className="glass-card ambient-shadow p-8 rounded-3xl">
          <h3 className="font-headline-sm text-headline-sm text-on-surface mb-6">Doctor Performance</h3>
            ...
            {DOCTORS.map((doc) => (
              <div key={doc.id} className="flex items-center gap-4 p-3 hover:bg-primary-container/20 rounded-2xl transition-all">
                ...
                <div className="flex-1">
                  ...
                  <p className="font-body-sm text-body-sm text-on-surface-variant">{doc.patientsSeen} Patients seen</p>
                </div>
      ...
      <section className="glass-card ambient-shadow rounded-3xl overflow-hidden">
        <div className="px-8 py-6 border-b border-outline-variant/30 flex justify-between items-center bg-white/50">
          <h3 className="font-headline-sm text-headline-sm text-on-surface">Recent High-Value Transactions</h3>
          <button className="text-primary font-label-md flex items-center gap-2 hover:opacity-80">View All <span className="material-symbols-outlined text-sm">arrow_forward</span></button>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead>
              <tr className="bg-primary-container/10 border-b border-outline-variant/30">
                <th className="px-8 py-4 font-label-md text-label-md text-on-surface-variant uppercase tracking-wider">Patient Name</th>
                <th className="px-8 py-4 font-label-md text-label-md text-on-surface-variant uppercase tracking-wider">Service</th>
                <th className="px-8 py-4 font-label-md text-label-md text-on-surface-variant uppercase tracking-wider">Amount</th>
